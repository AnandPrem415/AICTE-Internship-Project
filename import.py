import cv2
import os
import tkinter as tk
from tkinter import simpledialog
from tkinter import filedialog

def generate_lookup_tables():
    encrypt_table = {chr(i): i for i in range(256)}
    decrypt_table = {i: chr(i) for i in range(256)}
    return encrypt_table, decrypt_table

def encrypt_image(img, message):
    n, m, z = 0, 0, 0
    for char in message:
        img[n, m, z] = ord(char)
        n += 1
        m += 1
        z = (z + 1) % 3

def decrypt_message(img):
    n, m, z = 0, 0, 0
    decrypted_message = ""
    for _ in range(len(msg)):
        decrypted_message += chr(img[n, m, z])
        n += 1
        m += 1
        z = (z + 1) % 3
    return decrypted_message

def open_image():
    file_path = tk.filedialog.askopenfilename(title="Select Image File", filetypes=[("Image files", ".png;.jpg;*.jpeg")])
    img = cv2.imread(file_path)
    if img is None:
        tk.messagebox.showerror("Error", "Failed to load the image.")
    return img

def encrypt_and_save():
    global img, msg, encrypt_password_entry, decrypt_password_entry, decrypt_button
    img = open_image()
    if img is not None:
        msg = msg_entry.get()
        encrypt_image(img, msg)
        cv2.imwrite("encryptedImage.jpg", img)
        tk.messagebox.showinfo("Success", "Image encrypted and saved as encryptedImage.jpg.")
        
        # Hide encryption widgets
        msg_label.pack_forget()
        msg_entry.pack_forget()
        encrypt_password_label.pack_forget()
        encrypt_password_entry.pack_forget()
        encrypt_button.pack_forget()
        
        # Show decryption widgets
        decrypt_password_label.pack(pady=5)
        decrypt_password_entry.pack(pady=5)
        decrypt_button.pack(pady=10)

    msg_entry.delete(0, tk.END)  # Clear the message entry

def decrypt_and_display():
    global img, msg, encrypt_password_entry
    decrypt_password = decrypt_password_entry.get()
    if decrypt_password == encrypt_password_entry.get():
        decrypted_msg = decrypt_message(img)
        tk.messagebox.showinfo("Decryption Result", f"Decrypted message: {decrypted_msg}")
    else:
        tk.messagebox.showerror("Error", "Incorrect password.")

# Main GUI window
root = tk.Tk()
root.title("Image Encryption/Decryption")

# Set the window size
root.geometry("400x300")

# Dark-themed look
root.configure(bg="#333333")  # Set the background color

# Text Entry Boxes
msg_label = tk.Label(root, text="Enter Secret Message:", bg="#333333", fg="#eeeeee")
msg_label.pack(pady=5)
msg_entry = tk.Entry(root, bg="#555555", fg="#eeeeee")
msg_entry.pack(pady=5)

encrypt_password_label = tk.Label(root, text="Enter Encryption Passcode:", bg="#333333", fg="#eeeeee")
encrypt_password_label.pack(pady=5)
encrypt_password_entry = tk.Entry(root, show="*", bg="#555555", fg="#eeeeee")
encrypt_password_entry.pack(pady=5)

decrypt_password_label = tk.Label(root, text="Enter Decryption Passcode:", bg="#333333", fg="#eeeeee")
decrypt_password_label.pack(pady=5)
decrypt_password_entry = tk.Entry(root, show="*", bg="#555555", fg="#eeeeee")

# Buttons
encrypt_button = tk.Button(root, text="Encrypt Image", command=encrypt_and_save, bg="#555555", fg="#eeeeee")
encrypt_button.pack(pady=10)

decrypt_button = tk.Button(root, text="Decrypt Image", command=decrypt_and_display, bg="#555555", fg="#eeeeee")

# Run the GUI
root.mainloop()